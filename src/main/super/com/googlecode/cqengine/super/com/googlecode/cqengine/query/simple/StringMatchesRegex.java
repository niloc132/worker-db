...
public class StringMatchesRegex<O, A extends CharSequence> extends SimpleQuery<O, A> {
     ...
     */
    public StringMatchesRegex(Attribute<O, A> attribute, RegExp regexPattern) {
        super(attribute);
    ...
    public boolean matchesValue(A aValue, QueryOptions queryOptions){
        return regexPattern.test(aValue.toString());
    }
    ...
    public boolean equals(Object o) {
        ...
        return this.attribute.equals(that.attribute)
                && this.regexPattern.getSource().equals(that.regexPattern.getSource())
                && this.regexPattern.getMultiline() == that.regexPattern.getMultiline()
                && this.regexPattern.getIgnoreCase() == that.regexPattern.getIgnoreCase()
                && this.regexPattern.getGlobal() == that.regexPattern.getGlobal();
    }

    @Override
    protected int calcHashCode() {
        int result = attribute.hashCode();
        result = 31 * result + regexPattern.getSource().hashCode();
        return result;
    }
    ...
    public String toString() {
        return "matchesRegex(" + asLiteral(super.getAttributeName()) +
                ", " + asLiteral(regexPattern.getSource()) + ")";
    }
}